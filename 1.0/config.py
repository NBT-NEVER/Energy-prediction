...
# 实验1.0的原始数据、处理后数据和数据切分目录
DATA_DIR = DATA_ROOT / "1.0"
...
# 训练记录、评估结果和预测输出文件
SCALER_JSON = OUT_MODEL_DIR / "scaler_1.0.json"
TRAINING_LOG_CSV = OUT_MODEL_DIR / "training_log_1.0.csv"
TUNING_RESULTS_CSV = OUT_MODEL_DIR / "tuning_results_1.0.csv"
EVALUATION_JSON = OUT_MODEL_DIR / "evaluation_1.0.json"
EVALUATION_CSV = OUT_MODEL_DIR / "evaluation_1.0.csv"
FLIGHT_ENERGY_SUMMARY_CSV = OUT_MODEL_DIR / "flight_energy_summary_1.0.csv"
POWER_BIN_EVALUATION_CSV = OUT_MODEL_DIR / "power_bin_evaluation_1.0.csv"
PREDICTION_CSV = PREDICTION_DIR / "test_predictions_1.0.csv"

# 可视化和自定义工况输出文件
LOSS_CURVE_FILE = FIGURE_DIR / "training" / "loss_curve_1.0.png"
...
CUSTOM_SCENARIO_CSV = CUSTOM_DIR / "custom_scenarios_1.0.csv"
CUSTOM_PREDICTION_CSV = CUSTOM_DIR / "custom_predictions_1.0.csv"
VISUALIZATION_SUMMARY_JSON = OUT_DIR / "visualization_summary_1.0.json"
...
def build_config(**overrides: object) -> ExperimentConfig:
    ...
    if "save_dir" in normalized:
        ...
        normalized.setdefault("best_model_file", save_dir / "best_energy_mlp_1.0.pt")
        normalized.setdefault("final_model_file", save_dir / "final_energy_mlp_1.0.pt")
    if "out_dir" in normalized:
        ...
        normalized.setdefault("scaler_json", out_model_dir / "scaler_1.0.json")
        normalized.setdefault("training_log_csv", out_model_dir / "training_log_1.0.csv")
        normalized.setdefault("tuning_results_csv", out_model_dir / "tuning_results_1.0.csv")
        normalized.setdefault("evaluation_json", out_model_dir / "evaluation_1.0.json")
        normalized.setdefault("evaluation_csv", out_model_dir / "evaluation_1.0.csv")
        normalized.setdefault("flight_energy_summary_csv", out_model_dir / "flight_energy_summary_1.0.csv")
        normalized.setdefault("power_bin_evaluation_csv", out_model_dir / "power_bin_evaluation_1.0.csv")
        normalized.setdefault("prediction_csv", prediction_dir / "test_predictions_1.0.csv")
        normalized.setdefault("loss_curve_file", figure_dir / "training" / "loss_curve_1.0.png")
        ...
        normalized.setdefault("custom_scenario_csv", custom_dir / "custom_scenarios_1.0.csv")
        normalized.setdefault("custom_prediction_csv", custom_dir / "custom_predictions_1.0.csv")
        normalized.setdefault("visualization_summary_json", out_dir / "visualization_summary_1.0.json")
