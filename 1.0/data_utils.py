...
# 开发时间: 2026-07-07
# 文件名: data_utils.py
# 功能说明: 下载无人机公开数据集并完成能耗预测特征工程、数据清洗和数据切分
# 版本号：1.0
...
def add_derived_features(raw_frame: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    ...
    # 只保留真实飞行、悬停等空中工况，剔除A类地面测试。
    frame = frame[frame["route"].str.startswith(("R", "H"))].copy()
...
def prepare_dataset(cfg: ExperimentConfig, force: bool = False) -> dict:
    ...
    summary = {
        "_文件说明": (
            "本文件汇总实验1.0的数据来源、清洗结果、数据集划分规模、模型输入维度和预测目标。"
        ...
        "_字段说明": {
            ...
            "processed_rows": (
                "完成数值转换、缺失值删除、R/H类航线筛选、特征工程及无穷值清理后"
                "保留的有效采样记录总数，单位为行。"
            ),
            "processed_flights": (
