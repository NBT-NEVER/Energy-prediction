...
# 开发时间: 2026-07-08
# 文件名: visualize.py
# 功能说明: 生成无人机能耗模型训练、评估、预测和自定义工况预测可视化图表
# 版本号：1.0
...
def plot_result_summary(cfg: ExperimentConfig) -> list[Path]:
    ...
    if cfg.evaluation_json.exists():
        ...
        plt.figure(figsize=(9, 5))
        plt.bar(names, values, color=["#4c78a8", "#72b7b2", "#f58518", "#54a24b"])
...
def predict_custom_scenario(
    ...
    x = custom_frame[feature_columns].to_numpy(dtype=np.float32)
    custom_frame["predicted_power_w"] = predict_array(model, x, scaler, device, cfg.batch_size)
    ...
    save_json(cfg.custom_dir / "custom_prediction_summary_1.0.json", summary)
    return summary
