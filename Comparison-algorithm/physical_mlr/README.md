...
本算法与其他方法使用同一 R1 源数据和同一完整 flight 划分：训练 198,469 条、126 个 flight；验证 41,052 条、28 个 flight；测试 45,237 条、28 个 flight。实际模型输入为：从公共字段构造 7 个当前时刻物理量；无序列窗口，使用单一全局回归及 moving 指示变量。测试功率目标统一为 `power_w`，能耗统一按每条记录的 `dt_seconds` 积分；测试集只用于最终报告，不参与候选选择。

本次最终测试结果：功率 MAE=50.533 W，RMSE=89.757 W，R²=0.8391，WAPE=12.437%；flight 能耗 MAE=0.693 Wh，RMSE=0.873 Wh，R²=0.9654，WAPE=3.169%。

## 3. 验证集调参记录

| 验证功率WAPE(%) | 验证能耗WAPE(%) | 选择分数 | 最终选择 | alpha |
|---:|---:|---:|---:|---:|
| 11.5786 | 2.1228 | 12.6400 | 是 | 0.0000 |
| 11.5786 | 2.1228 | 12.6400 | 否 | 0.0100 |
| 11.5786 | 2.1228 | 12.6400 | 否 | 0.1000 |
| 11.5787 | 2.1228 | 12.6401 | 否 | 1.0000 |
| 11.5797 | 2.1231 | 12.6413 | 否 | 10.0000 |
| 11.5901 | 2.1261 | 12.6531 | 否 | 100.0000 |

最终选择候选 `—`：`alpha=0.0000`。验证集选择分数为 `12.6400`；测试集没有参与候选筛选。
...
|---|---|---:|---:|---:|---:|
| idle | 停机/低速 | 10,616 | 49.94 | 123.66 | 88.09 |
| ascent | 上升 | 19,819 | 43.91 | 61.81 | 8.17 |
| descent | 下降 | 12,284 | 62.67 | 97.59 | 13.11 |
| cruise | 巡航 | 2,518 | 45.94 | 61.06 | 9.22 |
...
|---|---:|---:|---:|---:|
| 0–50 W | 9,300 | 38.47 | 111.53 | 2141.60 |
| 50–300 W | 859 | 258.38 | 284.87 | 157.90 |
| 300–450 W | 6,595 | 69.26 | 92.64 | 16.84 |
| 450–600 W | 22,870 | 35.98 | 53.69 | 6.95 |
| >600 W | 5,613 | 76.00 | 100.37 | 11.61 |

测试集中样本最多的是 `450–600 W`（22,870 条），它对总体 MAE 的贡献最大；当前算法的绝对误差最高区间是 `50–300 W`（MAE=258.38 W），通常对应起飞、降落或功率过渡段。

## 5. 图表与数据分析

该算法在测试集上的功率 MAE 为 **50.533 W**、RMSE 为 **89.757 W**、$R^2=0.8391$、WAPE 为 **12.437%**；flight 能耗 MAE 为 **0.693 Wh**、RMSE 为 **0.873 Wh**、WAPE 为 **3.169%**。残差均值为 3.214 W，样本标准差为 89.701 W，绝对误差 P50/P90/P95 为 29.54/107.81/159.32 W。下面逐图说明横纵轴、每条线或颜色、计算方式以及当前图中的数值。
...
图中显示等间隔抽样点，$R^2=0.8391$ 与 WAPE=12.437% 则由全部 45,237 条记录计算。WAPE 表示全部绝对误差之和占实测功率绝对值之和的比例。
...
图中前三个测试 flight 为 18、23、83；flight 18 的区间 MAE=47.68 W，flight 23 的区间 MAE=50.62 W，flight 83 的区间 MAE=73.28 W。同色实线/虚线分别是实测/预测，峰值错位会增加尾部误差。
...
残差均值 3.214 W 表明整体存在轻微高估倾向；标准差 89.70 W 与 RMSE 的差异共同反映了尖峰样本。
...
橙柱最高的区间是 50–300 W（MAE=258.38 W）；蓝线显示该区间只有 859 条样本，不能仅凭柱高判断其对总体指标的贡献。
...
28 个蓝点对应 28 个 flight；任务级 WAPE=3.169%，点到 $y=x$ 的垂直距离就是 `energy_error_wh`。
...
任务误差范围为 -1.354～1.990 Wh；红柱是高估，蓝柱是低估。
...
按 WAPE 看，当前算法误差最低的阶段是 ascent（上升，8.17%）；若某阶段 RMSE 明显高于 MAE，说明少量突变点主导了平方误差。
...
CDF 在 29.54 W、107.81 W 和 159.32 W 附近分别达到 0.50、0.90 和 0.95，表示对应比例的测试点误差不超过这些阈值。
...
最大绝对误差位于 flight 250、时间 20.40 s；实测/预测功率为 730.41/22.78 W，残差为 -707.63 W。上图显示该点前后最多各 100 条记录，下图给出同一窗口的残差符号和持续时间。
...
红色均值线对应平均任务误差 0.173 Wh；若它偏离黑色零线，说明误差不是单纯随机波动。
...
四阶段绝对误差中位数为 idle 12.32 W、ascent 32.48 W、descent 39.79 W、cruise 35.07 W。箱体高表示中间 50% 样本跨度大；图中隐藏离群点，完整尾部应结合 CDF 和 P95。
