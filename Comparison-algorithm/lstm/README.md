# 两层 BiLSTM（Bidirectional Long Short-Term Memory，双向长短期记忆网络）
...
本次最终测试结果：功率 MAE=40.908 W，RMSE=66.094 W，R²=0.9221，WAPE=10.466%；flight 能耗 MAE=0.817 Wh，RMSE=1.229 Wh，R²=0.9467，WAPE=3.630%。

## 3. 验证集调参记录

| 候选 | 验证最优轮次 | 验证功率WAPE(%) | 验证能耗WAPE(%) | 选择分数 | 最终选择 | hidden_size | layers | bidirectional | dropout | learning_rate | weight_decay |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 | 23 | 9.4645 | 2.1492 | 10.5391 | 否 | 64 | 2 | True | 0.3000 | 0.0010 | 0.0001 |
| 2 | 18 | 9.3908 | 1.7945 | 10.2880 | 是 | 128 | 2 | True | 0.5000 | 0.0010 | 0.0001 |

最终选择候选 `2`：`hidden_size=128.0000`、`layers=2.0000`、`bidirectional=1.0000`、`dropout=0.5000`、`learning_rate=0.0010`、`weight_decay=0.0001`。验证集选择分数为 `10.2880`；测试集没有参与候选筛选。
...
|---|---|---:|---:|---:|---:|
| idle | 停机/低速 | 14,086 | 15.94 | 47.88 | 22.74 |
| ascent | 上升 | 10,185 | 59.54 | 81.00 | 10.54 |
| descent | 下降 | 13,988 | 51.63 | 75.94 | 10.32 |
| cruise | 巡航 | 10,465 | 42.06 | 56.21 | 8.29 |
...
|---|---:|---:|---:|---:|
| 0–50 W | 11,783 | 4.65 | 16.02 | 301.32 |
| 50–300 W | 856 | 151.32 | 197.45 | 90.66 |
| 300–450 W | 6,779 | 63.31 | 80.88 | 15.36 |
| 450–600 W | 23,081 | 37.65 | 51.93 | 7.25 |
| >600 W | 6,225 | 82.04 | 105.95 | 12.44 |

测试集中样本最多的是 `450–600 W`（23,081 条），它对总体 MAE 的贡献最大；当前算法的绝对误差最高区间是 `50–300 W`（MAE=151.32 W），通常对应起飞、降落或功率过渡段。

## 5. 图表与数据分析

该算法在测试集上的功率 MAE 为 **40.908 W**、RMSE 为 **66.094 W**、$R^2=0.9221$、WAPE 为 **10.466%**；flight 能耗 MAE 为 **0.817 Wh**、RMSE 为 **1.229 Wh**、WAPE 为 **3.630%**。残差均值为 2.154 W，样本标准差为 66.059 W，绝对误差 P50/P90/P95 为 24.03/100.98/136.82 W。下面逐图说明横纵轴、每条线或颜色、计算方式以及当前图中的数值。
...
图中显示等间隔抽样点，$R^2=0.9221$ 与 WAPE=10.466% 则由全部 48,724 条记录计算。WAPE 表示全部绝对误差之和占实测功率绝对值之和的比例。
...
图中前三个测试 flight 为 1、5、6；flight 1 的区间 MAE=32.23 W，flight 5 的区间 MAE=40.46 W，flight 6 的区间 MAE=38.10 W。同色实线/虚线分别是实测/预测，峰值错位会增加尾部误差。
...
残差均值 2.154 W 表明整体存在轻微高估倾向；标准差 66.06 W 与 RMSE 的差异共同反映了尖峰样本。
...
橙柱最高的区间是 50–300 W（MAE=151.32 W）；蓝线显示该区间只有 856 条样本，不能仅凭柱高判断其对总体指标的贡献。
...
47 个蓝点对应 47 个 flight；任务级 WAPE=3.630%，点到 $y=x$ 的垂直距离就是 `energy_error_wh`。
...
任务误差范围为 -2.397～5.252 Wh；红柱是高估，蓝柱是低估。
...
按 WAPE 看，当前算法误差最低的阶段是 cruise（巡航，8.29%）；若某阶段 RMSE 明显高于 MAE，说明少量突变点主导了平方误差。
...
CDF 在 24.03 W、100.98 W 和 136.82 W 附近分别达到 0.50、0.90 和 0.95，表示对应比例的测试点误差不超过这些阈值。
...
最大绝对误差位于 flight 84、时间 15.40 s；实测/预测功率为 674.55/13.99 W，残差为 -660.56 W。上图显示该点前后最多各 100 条记录，下图给出同一窗口的残差符号和持续时间。
...
红色均值线对应平均任务误差 0.124 Wh；若它偏离黑色零线，说明误差不是单纯随机波动。
...
四阶段绝对误差中位数为 idle 2.95 W、ascent 44.29 W、descent 34.91 W、cruise 32.12 W。箱体高表示中间 50% 样本跨度大；图中隐藏离群点，完整尾部应结合 CDF 和 P95。
...
适合规划位置、风场和载荷随时间连续变化的短窗功率估计。双向分支在已缓存的20步历史至当前窗口内从两端编码，能够利用窗口内部上下文；部署时仍需先缓存完整窗口，且本实现不会读取预测时刻之后的样本。 <sup>[A3]</sup>
