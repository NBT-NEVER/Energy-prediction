...
本算法与其他方法使用同一 R1 源数据和同一完整 flight 划分：训练 198,469 条、126 个 flight；验证 41,052 条、28 个 flight；测试 45,237 条、28 个 flight。实际模型输入为：23 个公共特征先按 flight 做 10 点历史 SMA，再组成 17 步（约 2.0 s）窗口。测试功率目标统一为 `power_w`，能耗统一按每条记录的 `dt_seconds` 积分；测试集只用于最终报告，不参与候选选择。

本次最终测试结果：功率 MAE=31.304 W，RMSE=47.853 W，R²=0.9543，WAPE=7.705%；flight 能耗 MAE=0.532 Wh，RMSE=0.725 Wh，R²=0.9762，WAPE=2.430%。

## 3. 验证集调参记录

| 候选 | 验证最优轮次 | 验证功率WAPE(%) | 验证能耗WAPE(%) | 选择分数 | 最终选择 | channels | blocks | dropout | sma_window | learning_rate | weight_decay |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 | 11 | 7.0467 | 1.7201 | 7.9068 | 否 | 48 | 3 | 0.0800 | 5 | 0.0020 | 0.0001 |
| 2 | 24 | 6.7456 | 1.8506 | 7.6709 | 是 | 64 | 3 | 0.1200 | 10 | 0.0010 | 0.0001 |

最终选择候选 `2`：`channels=64.0000`、`blocks=3.0000`、`dropout=0.1200`、`sma_window=10.0000`、`learning_rate=0.0010`、`weight_decay=0.0001`。验证集选择分数为 `7.6709`；测试集没有参与候选筛选。
...
|---|---|---:|---:|---:|---:|
| idle | 停机/低速 | 10,616 | 12.26 | 34.58 | 21.63 |
| ascent | 上升 | 19,819 | 36.61 | 49.21 | 6.81 |
| descent | 下降 | 12,284 | 39.05 | 55.62 | 8.17 |
| cruise | 巡航 | 2,518 | 32.10 | 44.14 | 6.44 |
...
|---|---:|---:|---:|---:|
| 0–50 W | 9,300 | 5.71 | 18.11 | 317.69 |
| 50–300 W | 859 | 92.54 | 118.12 | 56.56 |
| 300–450 W | 6,595 | 51.27 | 68.41 | 12.47 |
| 450–600 W | 22,870 | 29.43 | 39.45 | 5.69 |
| >600 W | 5,613 | 48.51 | 62.75 | 7.41 |

测试集中样本最多的是 `450–600 W`（22,870 条），它对总体 MAE 的贡献最大；当前算法的绝对误差最高区间是 `50–300 W`（MAE=92.54 W），通常对应起飞、降落或功率过渡段。

## 5. 图表与数据分析

该算法在测试集上的功率 MAE 为 **31.304 W**、RMSE 为 **47.853 W**、$R^2=0.9543$、WAPE 为 **7.705%**；flight 能耗 MAE 为 **0.532 Wh**、RMSE 为 **0.725 Wh**、WAPE 为 **2.430%**。残差均值为 5.560 W，样本标准差为 47.530 W，绝对误差 P50/P90/P95 为 20.62/74.82/98.97 W。下面逐图说明横纵轴、每条线或颜色、计算方式以及当前图中的数值。
...
图中显示等间隔抽样点，$R^2=0.9543$ 与 WAPE=7.705% 则由全部 45,237 条记录计算。WAPE 表示全部绝对误差之和占实测功率绝对值之和的比例。
...
图中前三个测试 flight 为 18、23、83；flight 18 的区间 MAE=31.07 W，flight 23 的区间 MAE=33.71 W，flight 83 的区间 MAE=41.87 W。同色实线/虚线分别是实测/预测，峰值错位会增加尾部误差。
...
残差均值 5.560 W 表明整体存在轻微高估倾向；标准差 47.53 W 与 RMSE 的差异共同反映了尖峰样本。
...
橙柱最高的区间是 50–300 W（MAE=92.54 W）；蓝线显示该区间只有 859 条样本，不能仅凭柱高判断其对总体指标的贡献。
...
28 个蓝点对应 28 个 flight；任务级 WAPE=2.430%，点到 $y=x$ 的垂直距离就是 `energy_error_wh`。
...
任务误差范围为 -0.688～2.313 Wh；红柱是高估，蓝柱是低估。
...
按 WAPE 看，当前算法误差最低的阶段是 cruise（巡航，6.44%）；若某阶段 RMSE 明显高于 MAE，说明少量突变点主导了平方误差。
...
CDF 在 20.62 W、74.82 W 和 98.97 W 附近分别达到 0.50、0.90 和 0.95，表示对应比例的测试点误差不超过这些阈值。
...
最大绝对误差位于 flight 135、时间 15.00 s；实测/预测功率为 582.88/13.23 W，残差为 -569.65 W。上图显示该点前后最多各 100 条记录，下图给出同一窗口的残差符号和持续时间。
...
红色均值线对应平均任务误差 0.299 Wh；若它偏离黑色零线，说明误差不是单纯随机波动。
...
四阶段绝对误差中位数为 idle 1.93 W、ascent 28.18 W、descent 27.56 W、cruise 23.86 W。箱体高表示中间 50% 样本跨度大；图中隐藏离群点，完整尾部应结合 CDF 和 P95。
