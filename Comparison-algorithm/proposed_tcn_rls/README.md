# 4.1 TCN（Temporal Convolutional Network，时间卷积网络）+ RLS（Recursive Least Squares，递推最小二乘）主方法

**算法来源：** 本项目 4.1 实现；TCN（Temporal Convolutional Network，时间卷积网络）与 RLS（Recursive Least Squares，递推最小二乘）理论分别参考 Bai 等和 Sayed，实验图表组织参考 Luo 等人的 RF-TLATT 研究。正文引用 <sup>[A0]</sup><sup>[A6]</sup><sup>[A7]</sup>。
...
本算法与其他方法使用同一 4.1 十维任务前数据和同一完整 flight 划分：训练 113,766 条、124 个 flight；验证 26,300 条、29 个 flight；测试 48,724 条、47 个 flight。实际模型输入为：10 个4.1任务前特征，20 步（4 s）历史至当前窗口；末端 TCN 输出再由上一完整秒窗更新的 RLS 参数校正。测试功率目标统一为 `power_w`，能耗统一按每条记录的 `dt_seconds` 积分；测试集只用于最终报告，不参与候选选择。

本次最终测试结果：功率 MAE=37.123 W，RMSE=60.561 W，R²=0.9346，WAPE=9.498%；flight 能耗 MAE=0.215 Wh，RMSE=0.534 Wh，R²=0.9899，WAPE=0.956%。

## 3. 验证集调参记录

本算法为固定 4.1 checkpoint 或无可调候选，未单独生成候选表。
...
|---|---|---:|---:|---:|---:|
| idle | 停机/低速 | 14,086 | 18.82 | 50.80 | 26.85 |
| ascent | 上升 | 10,185 | 52.87 | 73.72 | 9.36 |
| descent | 下降 | 13,988 | 45.56 | 67.42 | 9.11 |
| cruise | 巡航 | 10,465 | 35.15 | 47.30 | 6.93 |
...
|---|---:|---:|---:|---:|
| 0–50 W | 11,783 | 8.69 | 25.84 | 562.89 |
| 50–300 W | 856 | 156.43 | 203.88 | 93.73 |
| 300–450 W | 6,779 | 54.18 | 73.66 | 13.14 |
| 450–600 W | 23,081 | 33.75 | 46.50 | 6.50 |
| >600 W | 6,225 | 68.46 | 88.32 | 10.38 |

测试集中样本最多的是 `450–600 W`（23,081 条），它对总体 MAE 的贡献最大；当前算法的绝对误差最高区间是 `50–300 W`（MAE=156.43 W），通常对应起飞、降落或功率过渡段。

## 5. 图表与数据分析

该算法在测试集上的功率 MAE 为 **37.123 W**、RMSE 为 **60.561 W**、$R^2=0.9346$、WAPE 为 **9.498%**；flight 能耗 MAE 为 **0.215 Wh**、RMSE 为 **0.534 Wh**、WAPE 为 **0.956%**。残差均值为 2.869 W，样本标准差为 60.493 W，绝对误差 P50/P90/P95 为 21.83/89.61/123.56 W。下面逐图说明横纵轴、每条线或颜色、计算方式以及当前图中的数值。
...
图中显示等间隔抽样点，$R^2=0.9346$ 与 WAPE=9.498% 则由全部 48,724 条记录计算。WAPE 表示全部绝对误差之和占实测功率绝对值之和的比例。
...
图中前三个测试 flight 为 1、5、6；flight 1 的区间 MAE=30.46 W，flight 5 的区间 MAE=28.99 W，flight 6 的区间 MAE=28.05 W。同色实线/虚线分别是实测/预测，峰值错位会增加尾部误差。
...
残差均值 2.869 W 表明整体存在轻微高估倾向；标准差 60.49 W 与 RMSE 的差异共同反映了尖峰样本。
...
橙柱最高的区间是 50–300 W（MAE=156.43 W）；蓝线显示该区间只有 856 条样本，不能仅凭柱高判断其对总体指标的贡献。
...
47 个蓝点对应 47 个 flight；任务级 WAPE=0.956%，点到 $y=x$ 的垂直距离就是 `energy_error_wh`。
...
任务误差范围为 -0.276～3.425 Wh；红柱是高估，蓝柱是低估。
...
按 WAPE 看，当前算法误差最低的阶段是 cruise（巡航，6.93%）；若某阶段 RMSE 明显高于 MAE，说明少量突变点主导了平方误差。
...
CDF 在 21.83 W、89.61 W 和 123.56 W 附近分别达到 0.50、0.90 和 0.95，表示对应比例的测试点误差不超过这些阈值。
...
最大绝对误差位于 flight 243、时间 179.00 s；实测/预测功率为 131.79/1028.99 W，残差为 +897.20 W。上图显示该点前后最多各 100 条记录，下图给出同一窗口的残差符号和持续时间。
...
红色均值线对应平均任务误差 0.165 Wh；若它偏离黑色零线，说明误差不是单纯随机波动。
...
四阶段绝对误差中位数为 idle 1.90 W、ascent 37.78 W、descent 30.87 W、cruise 26.65 W。箱体高表示中间 50% 样本跨度大；图中隐藏离群点，完整尾部应结合 CDF 和 P95。
