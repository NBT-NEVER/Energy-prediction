...
# 开发时间: 2026-09-16
# 文件名: task_api.py
# 功能说明: 提供实验4.0任务前基础动力能耗预测和飞行中剩余路线RLS修正接口
# 版本号：4.0
...
REQUIRED_PLANNING_COLUMNS = [
    ...
    "planned_heading_deg", "wind_speed_mps", "wind_direction_deg", "payload_kg",
    "wind_forecast_source", "wind_forecast_timestamp", "wind_speed_error_mps",
    "wind_direction_error_deg", "wind_coordinate_system", "wind_direction_convention",
]
...
def validate_task_input(task: pd.DataFrame, cfg: ExperimentConfig) -> None:
    """功能: 检查任务前规划器输入是否满足4.0的物理字段与0.2秒契约。
    ...
    text_columns = {
        "task_id", "wind_forecast_source", "wind_forecast_timestamp",
        "wind_coordinate_system", "wind_direction_convention",
    }
    numeric_columns = [column for column in REQUIRED_PLANNING_COLUMNS if column not in text_columns]
    numeric_columns += [column for column in ("auxiliary_power_w",) if column in task]
    ...
    dt = numeric["dt_seconds"].to_numpy(float)
    if not np.allclose(dt, cfg.resample_seconds, atol=1e-6):
        raise ValueError(f"任务前输入必须统一使用{cfg.resample_seconds:g}s离散步长")
    if (numeric["wind_speed_mps"] < 0).any() or (numeric["payload_kg"] < 0).any():
        raise ValueError("wind_speed_mps和payload_kg不得为负")
    if (numeric[["wind_speed_error_mps", "wind_direction_error_deg"]] < 0).any().any():
        raise ValueError("风预报误差范围不得为负")
    if "auxiliary_power_w" in numeric and (numeric["auxiliary_power_w"] < 0).any():
        raise ValueError("auxiliary_power_w必须是非负的已知设备额定功率")
    if task["task_id"].isna().any():
        raise ValueError("task_id不能为空")
    for column in text_columns - {"task_id"}:
        if task[column].isna().any() or task[column].astype(str).str.strip().eq("").any():
            raise ValueError(f"{column}不能为空")
    allowed_coordinates = {"ENU_TRUE_NORTH", "ENU_MAGNETIC_NORTH", "PLANNER_LOCAL"}
    coordinates = set(task["wind_coordinate_system"].astype(str).str.strip().str.upper())
    if not coordinates.issubset(allowed_coordinates):
        raise ValueError(f"wind_coordinate_system仅支持{sorted(allowed_coordinates)}")
    allowed_conventions = {"VECTOR_TO", "METEOROLOGICAL_FROM"}
    conventions = set(task["wind_direction_convention"].astype(str).str.strip().str.upper())
    if not conventions.issubset(allowed_conventions):
        raise ValueError(f"wind_direction_convention仅支持{sorted(allowed_conventions)}")
    pd.to_datetime(task["wind_forecast_timestamp"], errors="raise", utc=True)
    for task_id, group in task.groupby("task_id", sort=False):
        for column in ("wind_coordinate_system", "wind_direction_convention"):
            if group[column].astype(str).str.strip().str.upper().nunique() != 1:
                raise ValueError(f"任务{task_id}的{column}必须保持一致")
...
def planning_to_model_frame(task: pd.DataFrame, cfg: ExperimentConfig) -> pd.DataFrame:
    """功能: 将规划器的三轴轨迹与风场字段转换为当前特征契约的TCN输入。
    ...
    result["route"] = result["route"].astype(str) if "route" in result else "PLANNER_ROUTE"
    result["time"] = pd.to_numeric(result["time_s"], errors="raise").astype(float)
    result["auxiliary_power_w"] = (pd.to_numeric(result["auxiliary_power_w"], errors="raise")
                                    if "auxiliary_power_w" in result else 0.0)
    groups = []
    for _, group in result.groupby("task_id", sort=False):
        group = group.copy()
        start_time = float(group["time_s"].iloc[0])
        relative_time = group["time_s"].to_numpy(float) - start_time
        duration = max(float(relative_time[-1]), cfg.resample_seconds)
        group["time_s"] = relative_time
        group["time"] = relative_time
        group["task_progress"] = np.clip(relative_time / duration, 0.0, 1.0)
        group["task_duration_s"] = duration
        vx = group["planned_vx_mps"].to_numpy(float)
        vy = group["planned_vy_mps"].to_numpy(float)
        vz = group["planned_vz_mps"].to_numpy(float)
        speed_norm = np.sqrt(vx ** 2 + vy ** 2 + vz ** 2)
        group["segment_distance_m"] = speed_norm * group["dt_seconds"].to_numpy(float)
        group["cumulative_distance_m"] = group["segment_distance_m"].cumsum()
        altitude_delta = group["planned_altitude_m"].diff().fillna(0.0)
        group["cumulative_climb_m"] = altitude_delta.clip(lower=0.0).cumsum()
        group["cumulative_descent_m"] = (-altitude_delta.clip(upper=0.0)).cumsum()
        wind_global_deg = group["wind_direction_deg"].to_numpy(float)
        meteorological = group["wind_direction_convention"].astype(str).str.upper().eq("METEOROLOGICAL_FROM").to_numpy()
        wind_global_deg = np.mod(wind_global_deg + np.where(meteorological, 180.0, 0.0), 360.0)
        wind_angle = np.deg2rad(group["planned_heading_deg"].to_numpy(float) - wind_global_deg)
        wind_speed = group["wind_speed_mps"].to_numpy(float)
        # 转为机体系相对风角后，X轴沿机头，Y轴横向。
        ...
        group["headwind_mps"] = -(vx * wind_x + vy * wind_y) / np.maximum(horizontal_speed, 1e-6)
        group["crosswind_mps"] = np.abs(vx * wind_y - vy * wind_x) / np.maximum(horizontal_speed, 1e-6)
        groups.append(group)
    ...
    columns = ["task_id", "flight", "route", "time", "auxiliary_power_w", *FEATURE_COLUMNS]
...
def _conditional_residual_std(model_frame: pd.DataFrame, cfg: ExperimentConfig) -> np.ndarray:
    """功能: 按风场、载荷、任务长度、进度和规划速度生成逐时刻残差标准差。
    ...
    if not cfg.uncertainty_json.exists():
        raise FileNotFoundError(f"缺少任务前不确定性模型，请先完成4.0训练: {cfg.uncertainty_json}")
    ...
    speed = np.sqrt(
        model_frame["planned_vx_mps"].to_numpy(float) ** 2
        + model_frame["planned_vy_mps"].to_numpy(float) ** 2
        + model_frame["planned_vz_mps"].to_numpy(float) ** 2
    )
    design = np.column_stack([
        model_frame["wind_speed_mps"].to_numpy(float), model_frame["wind_sin"].to_numpy(float),
        model_frame["wind_cos"].to_numpy(float), model_frame["payload_kg"].to_numpy(float),
        model_frame["task_duration_s"].to_numpy(float), model_frame["task_progress"].to_numpy(float), speed,
    ])
...
def _task_energy_multiplier(cfg: ExperimentConfig, confidence: float) -> float:
    ...
    if not cfg.uncertainty_json.exists():
        raise FileNotFoundError(f"缺少任务级区间校准模型，请先完成4.0训练: {cfg.uncertainty_json}")
    model = json.loads(cfg.uncertainty_json.read_text(encoding="utf-8"))
...
def predict_task_before(task: pd.DataFrame, cfg: ExperimentConfig, confidence: float | None = None,
    ...
    """功能: 输出单条候选路线的基础动力、附加设备、总能耗、任务级区间和电池决策。
    ...
    # 风预报误差范围只用于放大任务前不确定性，不作为功率标签或伪功率输入。
    ordered_task = task.sort_values("time_s", kind="stable").reset_index(drop=True)
    wind_speed = ordered_task["wind_speed_mps"].to_numpy(float)
    speed_error = ordered_task["wind_speed_error_mps"].to_numpy(float)
    direction_error = np.deg2rad(ordered_task["wind_direction_error_deg"].to_numpy(float))
    forecast_factor = np.sqrt(
        1.0 + (speed_error / np.maximum(wind_speed, 1.0)) ** 2 + direction_error ** 2
    )
    sigma_power = sigma_power * np.maximum(forecast_factor, 1.0)
    ...
    summary = {
        "version": "4.0", "task_id": str(task["task_id"].iloc[0]), "step_count": int(len(output)),
        ...
        "wind_forecast": {
            "sources": sorted(task["wind_forecast_source"].astype(str).unique().tolist()),
            "timestamps": sorted(task["wind_forecast_timestamp"].astype(str).unique().tolist()),
            "coordinate_systems": sorted(task["wind_coordinate_system"].astype(str).unique().tolist()),
            "direction_conventions": sorted(task["wind_direction_convention"].astype(str).unique().tolist()),
            "max_speed_error_mps": float(pd.to_numeric(task["wind_speed_error_mps"]).max()),
            "max_direction_error_deg": float(pd.to_numeric(task["wind_direction_error_deg"]).max()),
        },
    }
...
class OnlineTaskSession:
    ...
    参数: task为单条任务规划表，cfg为4.0配置，rls_params为可选RLS参数。
    ...
    def update(self, window_index: int, actual_energy_wh: float,
        ...
        if not feasible:
            ...
            decision = "continue"
        summary = {
            "version": "4.0", "updated_window": int(window_index), "executed_end_row": int(end),
