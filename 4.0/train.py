...
TARGET_COLUMN = "base_power_w"
UNCERTAINTY_FEATURES = [
    "wind_speed_mps", "wind_sin", "wind_cos", "payload_kg",
    "task_duration_s", "task_progress", "planned_speed_norm_mps",
]
...
def resolve_cuda_device(cfg: ExperimentConfig) -> torch.device:
    ...
    if not requested.startswith("cuda"):
        raise RuntimeError(f"实验4.0按约定只允许GPU训练和推理，当前device={cfg.device!r}")
    if not torch.cuda.is_available():
        raise RuntimeError("实验4.0要求使用GPU，但当前PyTorch未检测到可用CUDA设备")
...
def predict_frame(frame: pd.DataFrame, cfg: ExperimentConfig, checkpoint_path: Path | None = None) -> np.ndarray:
    ...
    if scaler.get("target_column") != TARGET_COLUMN or checkpoint.get("target_column") != TARGET_COLUMN:
        raise RuntimeError("当前scaler或模型权重不是以base_power_w为目标训练的，请重新执行4.0 GPU训练")
    if list(scaler.get("feature_columns", [])) != FEATURE_COLUMNS or list(checkpoint.get("feature_columns", [])) != FEATURE_COLUMNS:
        raise RuntimeError(f"当前scaler或模型权重与4.0的{len(FEATURE_COLUMNS)}维规划特征不一致，请重新训练")
...
def _uncertainty_design(frame: pd.DataFrame) -> np.ndarray:
    ...
    speed = np.sqrt(
        frame["planned_vx_mps"].to_numpy(float) ** 2
        + frame["planned_vy_mps"].to_numpy(float) ** 2
        + frame["planned_vz_mps"].to_numpy(float) ** 2
    )
    return np.column_stack([
        frame["wind_speed_mps"].to_numpy(float),
        frame["wind_sin"].to_numpy(float),
        frame["wind_cos"].to_numpy(float),
        frame["payload_kg"].to_numpy(float),
        frame["task_duration_s"].to_numpy(float),
        frame["task_progress"].to_numpy(float),
        speed,
    ])
