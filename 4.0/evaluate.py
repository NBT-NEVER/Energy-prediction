...
# 文件名: evaluate.py
# 开发时间: 2026-09-16
# 功能说明: 评估实验4.0规划TCN、窗口级动态RLS和未来任务能耗区间
# 版本号：4.0
...
FLIGHT_PROFILE_COLUMNS = [
    "flight", "route", "wind_speed_mps", "wind_sin", "wind_cos", "payload_kg", "task_duration_s",
]
...
def _validate_interval_calibration(cfg: ExperimentConfig) -> dict:
    ...
    if missing:
        raise RuntimeError(f"条件残差模型字段不完整，请重新训练4.0模型: {missing}")
    ...
    if list(model["condition_features"]) != UNCERTAINTY_FEATURES:
        raise RuntimeError("条件残差模型特征顺序与当前4.0实现不一致，请重新训练")
    ...
    if (len(model["feature_mean"]) != feature_count
        ...
        raise RuntimeError("条件残差模型参数维度与当前4.0实现不一致，请重新训练")
...
def _flight_input_profiles(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    profiles = source.groupby("flight", sort=True).agg(
        ...
        mean_wind_sin=("wind_sin", "mean"),
        mean_wind_cos=("wind_cos", "mean"),
        payload_kg=("payload_kg", "first"),
        task_duration_s=("task_duration_s", "first"),
    ).reset_index()
    profiles["mean_wind_direction_deg"] = np.mod(
        np.degrees(np.arctan2(profiles["mean_wind_sin"], profiles["mean_wind_cos"])), 360.0,
    )
    return profiles
...
def _save_stratified_evaluation(cfg: ExperimentConfig, train_profiles: pd.DataFrame,
    ...
    def append_group(stratification: str, stratum: str, rule: str,
        ...
        rows.append({
            ...
            "stratum": stratum,
            "selection_rule": rule,
            "selection_inputs": "route|mean_wind_speed_mps|wind_sin|wind_cos|payload_kg|task_duration_s",
    ...
    test_payload_unseen = test_profiles["payload_kg"].ge(UNSEEN_PAYLOAD_HOLDOUT_KG)
    train_payload_unseen = train_profiles["payload_kg"].ge(UNSEEN_PAYLOAD_HOLDOUT_KG)
    ...
    binary_specs = [
        ...
        (
            ...
            f"payload_kg >= {UNSEEN_PAYLOAD_HOLDOUT_KG:g}",
        ),
        (
            "duration_holdout",
    ...
    bin_specs = [
        ("mean_wind_speed_bin", "mean_wind_speed_mps", [-np.inf, 2.0, 4.0, 6.0, np.inf],
         ["<2", "2-<4", "4-<6", ">=6"], False),
        ("payload_bin", "payload_kg", [-np.inf, 0.25, 0.50, 0.75, np.inf],
         ["<0.25", "0.25-<0.50", "0.50-<0.75", ">=0.75"], False),
...
def _save_trace_outputs(cfg: ExperimentConfig, parameter_trace: list[dict],
    ...
    summary = {
        "version": "4.0", "parameter_trace_file": str(cfg.rls_parameter_trace_csv),
    ...
    if len(correction):
        ...
        correction_summary = {
            "version": "4.0", "rows": int(len(correction)),
        ...
        correction_summary = {"version": "4.0", "rows": 0, **interval_metrics}
...
def evaluate_model(cfg: ExperimentConfig) -> dict:
    ...
    flight = flight.merge(
        test_profiles[[
            "flight", "mean_wind_speed_mps", "mean_wind_direction_deg", "payload_kg", "task_duration_s",
        ]],
