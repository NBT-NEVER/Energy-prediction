...
# 文件名: visualize.py
# 开发时间: 2026-09-16
# 功能说明: 生成实验4.0训练、评估、预测、在线RLS修正和路线追踪可视化产物
# 版本号：4.0
...
def plot_training_outputs(cfg: ExperimentConfig) -> list[str]:
    ...
    if log is not None:
        ...
        axis.set_title("实验4.0正式TCN训练损失曲线")
        axis.legend()
        outputs.append(_save_svg(fig, output_dir / "loss_curve_4.0.svg"))
        ...
        if {"epoch", "learning_rate"}.issubset(final_log.columns):
            ...
            axis.set_xlabel("训练轮次")
            axis.set_ylabel("学习率")
            axis.set_title("实验4.0正式TCN学习率记录")
...
def plot_result_outputs(cfg: ExperimentConfig) -> list[str]:
    ...
    flight = _read_csv(cfg.out_model_dir / "flight_energy_summary_4.0.csv")
    power_bins = _read_csv(cfg.out_model_dir / "power_bin_evaluation_4.0.csv")
    ...
    if tuning is not None:
        ...
        outputs.append(_save_svg(fig, output_dir / "rls_grid_scores_4.0.svg"))
    return outputs
...
def plot_prediction_outputs(cfg: ExperimentConfig, maximum_flights: int = 3) -> list[str]:
    ...
    window_table = _read_csv(cfg.out_model_dir / "window_energy_summary_4.0.csv")
...
def plot_rls_correction_outputs(cfg: ExperimentConfig) -> list[str]:
    ...
    static_path = cfg.out_rls_dir / "rls_future_energy_correction_4.0.svg"
...
def _prepare_route_frame(group: pd.DataFrame) -> pd.DataFrame:
    ...
    frame["trajectory_x_m"] = _numeric(frame, ("planned_position_x_m", "position_x_m", "trajectory_x_m"))
    frame["trajectory_y_m"] = _numeric(frame, ("planned_position_y_m", "position_y_m", "trajectory_y_m"))
    altitude = _numeric(frame, ("planned_altitude_m", "position_z_m", "trajectory_z_m"))
    frame["trajectory_z_m"] = altitude - altitude[0] if len(altitude) else altitude
    vx = _numeric(frame, ("planned_vx_mps", "velocity_x"))
    vy = _numeric(frame, ("planned_vy_mps", "velocity_y"))
    vz = _numeric(frame, ("planned_vz_mps", "velocity_z"))
    frame["planned_speed_norm_mps"] = np.sqrt(vx ** 2 + vy ** 2 + vz ** 2)
    frame["wind_speed_plot_mps"] = _numeric(frame, ("wind_speed_mps", "wind_speed"))
    if "wind_direction_deg" in frame:
        direction = pd.to_numeric(frame["wind_direction_deg"], errors="coerce").fillna(0.0).to_numpy(float)
    elif {"wind_sin", "wind_cos"}.issubset(frame.columns):
        direction = np.degrees(np.arctan2(frame["wind_sin"].to_numpy(float),
                                          frame["wind_cos"].to_numpy(float))) % 360.0
    else:
        direction = np.zeros(len(frame), dtype=float)
    frame["wind_direction_plot_deg"] = direction
...
def _save_route_html(frame: pd.DataFrame, directory: Path, route: str, flight_id: object) -> str:
    custom_data = np.column_stack(
        [
            frame["plot_time_s"], frame["planned_speed_norm_mps"], frame["wind_speed_plot_mps"],
            frame["wind_direction_plot_deg"], _numeric(frame, ("planned_vx_mps",)),
            _numeric(frame, ("planned_vy_mps",)), _numeric(frame, ("planned_vz_mps",)),
        ]
    )
    figure = go.Figure()
    figure.add_trace(
        go.Scatter3d(
            ...
            hovertemplate=(
                ...
                "时间：%{customdata[0]:.1f} s<br>速度模长：%{customdata[1]:.2f} m/s<br>"
                "速度分量：(%{customdata[4]:.2f}, %{customdata[5]:.2f}, %{customdata[6]:.2f}) m/s<br>"
    ...
    path = directory / f"flight_{_flight_label(flight_id)}_imu_trajectory.html"
...
def _save_route_gif(frame: pd.DataFrame, directory: Path, route: str, flight_id: object) -> str:
    ...
    # 4.0 将原始飞行轨迹视为规划轨迹，任务执行按约束严格跟踪该轨迹；
    ...
    def update(frame_index: int):
        ...
        annotation.set_text(
            ...
            f"规划速度：({current['planned_vx_mps']:.2f}, {current['planned_vy_mps']:.2f}, "
            f"{current['planned_vz_mps']:.2f}) m/s\n"
            f"速度模长：{current['planned_speed_norm_mps']:.2f} m/s\n"
    ...
    path = directory / f"flight_{_flight_label(flight_id)}_imu_trajectory.gif"
...
def generate_route_products(cfg: ExperimentConfig) -> list[str]:
    ...
    for route, flight_id in selected.items():
        ...
        aligned_path = directory / f"flight_{_flight_label(flight_id)}_imu_aligned.csv"
    ...
    summary_path = cfg.out_route_dir / "route_products_summary_4.0.json"
    _save_json(
        summary_path,
        {
            "version": "4.0",
...
def generate_all_visualizations(cfg: ExperimentConfig) -> dict:
    """统一生成4.0的中文SVG、GIF、HTML、CSV与JSON追踪产物。"""
    ...
    categories = {
        ...
        "results": plot_result_outputs(cfg),
