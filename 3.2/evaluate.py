...
# 开发时间: 2026-07-07
# 文件名: evaluate.py
# 功能说明: 评估实验3.2的TCN基线和秒级能量监督RLS功率、能量误差
# 版本号：3.2
...
def evaluate_model(cfg: ExperimentConfig) -> dict:
    ...
    second_summary = predictions.groupby(["flight", "second_window"], sort=True).agg(
        actual_energy_wh=("actual_second_energy_wh", "first"),
        tcn_energy_wh=("tcn_second_energy_wh", "first"),
        predicted_energy_wh=("predicted_second_energy_wh", "first"),
    ...
    evaluation_json = {
        "_文件说明": "本文件汇总实验3.2测试集上的TCN原始输出、秒级能量监督RLS校正结果、预测区间覆盖率及相关输出文件路径。功率指标单位为W，能耗指标单位为Wh，百分比指标单位为%。",
        "_字段说明": evaluation_field_descriptions(),
        **metrics,
        "second_energy_evaluation_file": str(cfg.second_energy_evaluation_csv),
    }
