...
# 开发时间: 2026-09-07
# 文件名: train.py
# 功能说明: 执行实验3.2的TCN窗口选择、秒级能量监督RLS网格选择和最终训练
# 版本号：3.2
...
def build_rls_energy_window_ids(frame: pd.DataFrame, window_seconds: float) -> np.ndarray:
    """功能: 按RLS能量反馈时间区间生成窗口编号。
    参数: frame为含time列的数据表，window_seconds为RLS反馈窗口长度，单位s。
    返回: 与frame行对应的RLS能量窗口编号数组。
    调用位置: apply_rls_correction、predict.py。
    """

    window_seconds = float(window_seconds)
    if window_seconds <= 0:
        raise ValueError(f"RLS能量反馈窗口必须大于0秒，当前为{window_seconds}。")
    time_values = np.maximum(frame["time"].to_numpy(dtype=float), 0.0)
    return np.floor(time_values / window_seconds).astype(np.int64)
...
def apply_rls_correction(base_power: np.ndarray, frame: pd.DataFrame, scaler: dict, cfg: ExperimentConfig, initial_theta: list[float] | None = None, update: bool = True, progress_label: str | None = None, rls_params: dict | None = None, trace_path=None) -> tuple[np.ndarray, list[float]]:
    """功能: 按flight和RLS能量反馈时间区间执行先预测后更新的实时校正。
    ...
    rls_params = dict(rls_params or {})
    rls_params.setdefault("forgetting_factor", cfg.rls_forgetting_factor)
    rls_params.setdefault("initial_covariance", cfg.rls_initial_covariance)
    energy_window_seconds = float(rls_params.get("energy_window_seconds", cfg.rls_energy_window_seconds))
    ...
    window_ids = build_rls_energy_window_ids(ordered, energy_window_seconds)
    actual_values = ordered[cfg.target_column].to_numpy(dtype=float) if update and cfg.target_column in ordered.columns else None
    trace_rows: list[dict] = []
    processed_windows = 0
    minimum_complete_duration = 0.95 * energy_window_seconds
    for flight, flight_frame in ordered.groupby("flight", sort=False):
        ...
        for _, positions_frame in marked.groupby("_window", sort=True):
            ...
            duration_seconds = float(np.sum(dt))
            predicted_energy = float(np.sum(corrected[positions] * dt) / 3600.0)
            base_energy = float(np.sum(base_power[positions] * dt) / 3600.0)
            actual_energy = np.nan
            window_complete = int(duration_seconds >= minimum_complete_duration)
            if actual_values is not None:
                actual_energy = float(np.sum(actual_values[positions] * dt) / 3600.0)
                if window_complete:
                    # 当前窗口结束后才更新，更新结果只会影响下一个窗口。
                    corrector.update_window(base_energy, actual_energy, duration_seconds)
            trace_rows.append({
                "flight": int(flight),
                "second_window": int(np.floor(float(ordered.iloc[positions[0]]["time"]))),
                "rls_energy_window": int(window_ids[positions[0]]),
                "rls_energy_window_seconds": energy_window_seconds,
                "rls_window_start_seconds": float(ordered.iloc[positions[0]]["time"]),
                "rls_window_duration_seconds": duration_seconds,
                "rls_window_complete": window_complete,
                "flight_state": state,
                "state_changed": int(state_changed),
                "theta_bias_before": float(theta_before[0]),
                "theta_scale_before": float(theta_before[1]),
                "theta_bias_after": float(corrector.theta[0]),
                "theta_scale_after": float(corrector.theta[1]),
                "tcn_energy_wh": base_energy,
                "predicted_energy_wh": predicted_energy,
                "actual_energy_wh": actual_energy,
            })
            previous_state = state
            processed_windows += 1
            if progress and (processed_windows % 100 == 0 or int(positions[-1]) + 1 == len(frame)):
                progress.update(int(positions[-1]) + 1, f"已完成flight={flight}的RLS窗口{int(window_ids[positions[0]])}")
    if progress:
        progress.finish(f"RLS在线校正完成，反馈区间={energy_window_seconds:g}s")
    if trace_path is not None:
        trace_path.parent.mkdir(parents=True, exist_ok=True)
        trace_frame = pd.DataFrame(trace_rows)
        trace_frame.to_csv(trace_path, index=False, encoding="utf-8")
        if trace_path == cfg.rls_parameter_trace_csv:
            statistics_path = cfg.rls_parameter_statistics_csv
            summary_path = cfg.rls_parameter_summary_json
        else:
            statistics_path = trace_path.with_name(trace_path.stem.replace("_trace", "_statistics") + trace_path.suffix)
            summary_path = trace_path.with_name(trace_path.stem.replace("_trace", "_summary") + ".json")
        statistics = trace_frame.groupby("flight", sort=True).agg(
            window_count=("rls_energy_window", "count"),
            complete_window_count=("rls_window_complete", "sum"),
        ...
        statistics["rls_energy_window_seconds"] = energy_window_seconds
        statistics.to_csv(statistics_path, index=False, encoding="utf-8")
        save_json(summary_path, {
            "version": "3.2",
            "rls_energy_window_seconds": energy_window_seconds,
            "window_count": int(len(trace_frame)),
            "complete_window_count": int(trace_frame["rls_window_complete"].sum()),
...
def energy_window_selection_metrics(base_power: np.ndarray, corrected_power: np.ndarray, frame: pd.DataFrame, cfg: ExperimentConfig, energy_window_seconds: float | None = None) -> dict:
    """功能: 按指定RLS能量反馈区间计算所有flight的候选价值函数。
    参数: base_power和corrected_power为TCN及RLS逐点预测，frame为验证数据表，cfg为配置，energy_window_seconds为RLS反馈区间，单位s。
    返回: 按flight等权汇总的RLS能量窗口、逐点和整flight误差指标。
    调用位置: train_model的RLS超参数搜索。
    """

    energy_window_seconds = float(energy_window_seconds or cfg.rls_energy_window_seconds)
    work = frame[["flight", "time", "dt_seconds", cfg.target_column]].copy()
    work["window"] = build_rls_energy_window_ids(work, energy_window_seconds)
    ...
    return {
        "rls_energy_window_seconds": energy_window_seconds,
        "val_rls_energy_window_wape_mean": float(second_errors.mean()),
        # 保留旧字段名，便于读取历史脚本；其数值随当前RLS反馈区间变化。
...
def build_window_comparison_metrics(
    ...
    """功能: 计算固定RLS参数下单个能量窗的总体和flight级影响指标。
    参数: base_power和corrected_power为TCN与RLS功率，frame为验证或测试表，energy_window_seconds为对比窗长，stage为数据阶段，后两项为固定RLS参数。
    返回: 总体指标字典和逐flight明细表。
    调用位置: compare_rls_energy_windows。
    """

    actual = frame[cfg.target_column].to_numpy(dtype=float)
    sample_metrics = regression_metrics_local(actual, corrected_power, "sample_power_w")
    base_sample_metrics = regression_metrics_local(actual, base_power, "tcn_sample_power_w")
    work = frame[["flight", "time", "dt_seconds", cfg.target_column]].copy()
    work["tcn_power_w"] = base_power
    work["rls_power_w"] = corrected_power
    work["window"] = build_rls_energy_window_ids(work, energy_window_seconds)
    for prefix in ("actual", "tcn", "rls"):
        source = cfg.target_column if prefix == "actual" else f"{prefix}_power_w"
        work[f"{prefix}_energy_wh"] = work[source] * work["dt_seconds"] / 3600.0
    window = work.groupby(["flight", "window"], sort=True).agg(
        window_start_s=("time", "min"),
        window_duration_s=("dt_seconds", "sum"),
        actual_energy_wh=("actual_energy_wh", "sum"),
        tcn_energy_wh=("tcn_energy_wh", "sum"),
        rls_energy_wh=("rls_energy_wh", "sum"),
        tcn_mean_power_w=("tcn_power_w", "mean"),
        window_rows=("flight", "size"),
    ).reset_index()
    window["window_complete"] = (window["window_duration_s"] >= 0.95 * energy_window_seconds).astype(int)
    window["flight_state"] = (window["tcn_mean_power_w"] >= cfg.flight_state_threshold_w).astype(int)
    window["state_changed"] = window.groupby("flight", sort=False)["flight_state"].diff().fillna(0).ne(0).astype(int)
    window["tcn_error_wh"] = window["tcn_energy_wh"] - window["actual_energy_wh"]
    window["rls_error_wh"] = window["rls_energy_wh"] - window["actual_energy_wh"]
    window["tcn_abs_error_wh"] = window["tcn_error_wh"].abs()
    window["rls_abs_error_wh"] = window["rls_error_wh"].abs()
    window["tcn_abs_percent_error"] = window["tcn_abs_error_wh"] / window["actual_energy_wh"].abs().clip(lower=1e-6) * 100.0
    window["rls_abs_percent_error"] = window["rls_abs_error_wh"] / window["actual_energy_wh"].abs().clip(lower=1e-6) * 100.0
    window_metrics = regression_metrics_local(window["actual_energy_wh"].to_numpy(), window["rls_energy_wh"].to_numpy(), "rls_window_energy_wh")
    tcn_window_metrics = regression_metrics_local(window["actual_energy_wh"].to_numpy(), window["tcn_energy_wh"].to_numpy(), "tcn_window_energy_wh")
    complete_window = window[window["window_complete"].eq(1)].copy()
    complete_window_metrics = regression_metrics_local(
        complete_window["actual_energy_wh"].to_numpy(), complete_window["rls_energy_wh"].to_numpy(), "rls_complete_window_energy_wh"
    ) if not complete_window.empty else {key: 0.0 for key in regression_metrics_local(np.array([0.0]), np.array([0.0]), "rls_complete_window_energy_wh")}
    tcn_complete_window_metrics = regression_metrics_local(
        complete_window["actual_energy_wh"].to_numpy(), complete_window["tcn_energy_wh"].to_numpy(), "tcn_complete_window_energy_wh"
    ) if not complete_window.empty else {key: 0.0 for key in regression_metrics_local(np.array([0.0]), np.array([0.0]), "tcn_complete_window_energy_wh")}
    flight = window.groupby("flight", sort=True).agg(
        actual_energy_wh=("actual_energy_wh", "sum"),
        tcn_energy_wh=("tcn_energy_wh", "sum"),
        rls_energy_wh=("rls_energy_wh", "sum"),
        window_count=("window", "count"),
        complete_window_count=("window_complete", "sum"),
    ).reset_index()
    ...
    flight["tcn_abs_percent_error"] = flight["tcn_abs_error_wh"] / flight["actual_energy_wh"].abs().clip(lower=1e-6) * 100.0
    flight["rls_abs_percent_error"] = flight["rls_abs_error_wh"] / flight["actual_energy_wh"].abs().clip(lower=1e-6) * 100.0
    flight_metrics = regression_metrics_local(flight["actual_energy_wh"].to_numpy(), flight["rls_energy_wh"].to_numpy(), "rls_flight_energy_wh")
    tcn_flight_metrics = regression_metrics_local(flight["actual_energy_wh"].to_numpy(), flight["tcn_energy_wh"].to_numpy(), "tcn_flight_energy_wh")
    summary = {
        "version": "3.2", "stage": stage, "energy_window_seconds": float(energy_window_seconds),
        "rls_forgetting_factor": float(rls_forgetting_factor),
        "rls_initial_covariance": float(rls_initial_covariance), "rls_warmup_windows": 0,
        "sample_count": int(len(work)), "flight_count": int(len(flight)),
        "window_count": int(len(window)), "complete_window_count": int(window["window_complete"].sum()),
        **base_sample_metrics, **sample_metrics, **tcn_window_metrics, **window_metrics,
        **tcn_complete_window_metrics, **complete_window_metrics,
        **tcn_flight_metrics, **flight_metrics,
        "sample_power_wape_delta_vs_tcn_percent": float(sample_metrics["sample_power_w_wape_percent"] - base_sample_metrics["tcn_sample_power_w_wape_percent"]),
        "window_energy_wape_delta_vs_tcn_percent": float(window_metrics["rls_window_energy_wh_wape_percent"] - tcn_window_metrics["tcn_window_energy_wh_wape_percent"]),
        "flight_energy_wape_delta_vs_tcn_percent": float(flight_metrics["rls_flight_energy_wh_wape_percent"] - tcn_flight_metrics["tcn_flight_energy_wh_wape_percent"]),
        "window_abs_error_mean_wh": float(window["rls_abs_error_wh"].mean()),
        "window_abs_error_p95_wh": float(window["rls_abs_error_wh"].quantile(0.95)),
        "window_duration_mean_seconds": float(window["window_duration_s"].mean()),
        "window_duration_std_seconds": float(window["window_duration_s"].std(ddof=0)),
        "state_switch_count": int(window["state_changed"].sum()),
    }
    window["energy_window_seconds"] = float(energy_window_seconds)
    window["stage"] = stage
    flight["energy_window_seconds"] = float(energy_window_seconds)
    flight["stage"] = stage
    return summary, flight


def regression_metrics_local(y_true: np.ndarray, y_pred: np.ndarray, prefix: str) -> dict:
    """功能: 在训练模块内计算变量分析所需的回归指标。
    参数: y_true为真实值，y_pred为预测值，prefix为字段前缀。
    返回: MAE、RMSE、R2、MAPE和WAPE字典。
    调用位置: build_window_comparison_metrics。
    """
    ...
    return {
        ...
        f"{prefix}_r2": float(1.0 - np.sum(error ** 2) / denom) if denom > 1e-12 else 0.0,
        f"{prefix}_mape_percent": float(np.mean(np.abs(error[valid] / y_true[valid])) * 100.0) if valid.any() else 0.0,
        f"{prefix}_wape_percent": float(np.sum(np.abs(error)) / max(np.sum(np.abs(y_true)), 1e-6) * 100.0),
    }
...
def rls_candidate_grid(cfg: ExperimentConfig) -> list[dict]:
    ...
    return [
        {
            "candidate": f"rls_ff{forgetting:g}_cov{cov:g}",
...
def tune_rls_only(cfg: ExperimentConfig) -> dict:
    ...
    reference_window = float(cfg.rls_energy_window_seconds)
    print(f"仅搜索RLS：固定TCN={checkpoint.get('best_tcn_candidate', checkpoint.get('window_seconds'))}，参考能量窗固定为{reference_window:g}s（不参与搜索），候选数={len(candidates)}")
    for index, params in enumerate(candidates, start=1):
        candidate_started = time.perf_counter()
        runtime_params = {**params, "energy_window_seconds": reference_window}
        corrected, _ = apply_rls_correction(val_base, val_df, scaler, cfg, initial_theta, update=True, rls_params=runtime_params)
        metrics = energy_window_selection_metrics(val_base, corrected, val_df, cfg, reference_window)
        rows.append({"phase": "rls_only", "tcn_window_seconds": checkpoint["window_seconds"], "tcn_window_steps": window_steps, "reference_energy_window_seconds": reference_window, **params, **metrics})
        if metrics["selection_score"] < best_score:
            best_score = metrics["selection_score"]
            best_params = params.copy()
        elapsed = time.perf_counter() - candidate_started
        print(
            f"RLS候选 {index}/{len(candidates)} 测试结果："
            f"参考区间={reference_window:g}s，ff={params['forgetting_factor']:g}，cov={params['initial_covariance']:g}，"
            f"选择分数={metrics['selection_score']:.6f}，当前最优={best_score:.6f}，耗时={elapsed:.2f}s"
        )
    ...
    checkpoint.update({
        ...
        "rls_energy_window_seconds": reference_window,
    ...
    return {"version": "3.2", "mode": "tune-rls", "fixed_tcn_candidate": checkpoint.get("best_tcn_candidate"), "best_rls_candidate": best_params["candidate"], "rls_forgetting_factor": best_params["forgetting_factor"], "rls_initial_covariance": best_params["initial_covariance"], "reference_energy_window_seconds": reference_window, "rls_warmup_windows": 0, "best_rls_selection_score": best_score, "rls_candidate_count": len(rows)}


def compare_rls_energy_windows(cfg: ExperimentConfig) -> dict:
    """功能: 固定RLS遗忘因子和初始协方差，比较五种能量反馈窗的误差影响。
    参数: cfg为实验配置。
    返回: 验证集和测试集的能量窗变量分析摘要。
    调用位置: main.py的compare-rls-windows模式和all流程。
    """

    ensure_directories(cfg)
    if not cfg.best_model_file.exists():
        raise FileNotFoundError(f"未找到已训练模型: {cfg.best_model_file}")
    device = select_cuda_device(cfg.device)
    checkpoint = torch.load(cfg.best_model_file, map_location=device, weights_only=False)
    scaler = load_json(cfg.scaler_json)
    train_df, val_df, feature_columns = load_split_data(cfg)
    ...
    train_df = train_df.sort_values(["flight", "time"], kind="stable").reset_index(drop=True)
    val_df = val_df.sort_values(["flight", "time"], kind="stable").reset_index(drop=True)
    test_df = test_df.sort_values(["flight", "time"], kind="stable").reset_index(drop=True)
    model = build_model(int(checkpoint["input_dim"]), tuple(checkpoint["channels"]), float(checkpoint["dropout"]), int(checkpoint.get("kernel_size", 3)), checkpoint.get("dt_feature_index")).to(device)
    model.load_state_dict(checkpoint["model_state_dict"])
    window_steps = int(checkpoint["window_steps"])
    val_x, _ = build_sequence_arrays(val_df, scaler, window_steps, "RLS能量窗变量分析验证序列")
    test_x, _ = build_sequence_arrays(test_df, scaler, window_steps, "RLS能量窗变量分析测试序列")
    val_base = predict_original_power(model, val_x, scaler, device, cfg.batch_size, "RLS能量窗变量分析验证TCN推理")
    test_base = predict_original_power(model, test_x, scaler, device, cfg.batch_size, "RLS能量窗变量分析测试TCN推理")
    forgetting_factor = float(checkpoint.get("rls_forgetting_factor", cfg.rls_forgetting_factor))
    initial_covariance = float(checkpoint.get("rls_initial_covariance", cfg.rls_initial_covariance))
    rows: list[dict] = []
    flight_rows: list[pd.DataFrame] = []
    window_values = tuple(float(value) for value in cfg.rls_energy_window_candidates)
    print(f"RLS能量窗变量分析：固定ff={forgetting_factor:g}、cov={initial_covariance:g}、warmup=0，比较窗口={list(window_values)}s")
    for stage, frame, base_power in (("validation", val_df, val_base), ("test", test_df, test_base)):
        for index, window_seconds in enumerate(window_values, start=1):
            candidate_started = time.perf_counter()
            params = {
            ...
            trace_path = cfg.rls_dir / f"rls_window_comparison_{stage}_{str(window_seconds).replace('.', 'p')}s_trace_3.2.csv"
            corrected, _ = apply_rls_correction(base_power, frame, scaler, cfg, None, update=True, rls_params=params, progress_label=f"{stage} RLS {window_seconds:g}s", trace_path=trace_path)
            summary, flight = build_window_comparison_metrics(base_power, corrected, frame, cfg, window_seconds, stage, forgetting_factor, initial_covariance)
            trace = pd.read_csv(trace_path)
            for column, prefix in (("theta_bias_after", "rls_bias"), ("theta_scale_after", "rls_scale")):
                summary[f"{prefix}_mean"] = float(trace[column].mean())
                summary[f"{prefix}_std"] = float(trace[column].std(ddof=0))
                summary[f"{prefix}_min"] = float(trace[column].min())
                summary[f"{prefix}_max"] = float(trace[column].max())
            summary["parameter_boundary_touch_count"] = int(
                (trace["theta_bias_after"].abs() >= 0.999999).sum()
                + (trace["theta_scale_after"] <= 0.000001).sum()
                + (trace["theta_scale_after"] >= 1.999999).sum()
            )
            summary["trace_file"] = str(trace_path)
            rows.append(summary)
            flight["rls_forgetting_factor"] = forgetting_factor
            flight["rls_initial_covariance"] = initial_covariance
            flight_rows.append(flight)
            elapsed = time.perf_counter() - candidate_started
            print(
                f"RLS能量窗 {stage} {index}/{len(window_values)}：{window_seconds:g}s，"
                f"样本WAPE={summary['sample_power_w_wape_percent']:.4f}%、"
                f"窗口WAPE={summary['rls_window_energy_wh_wape_percent']:.4f}%、"
                f"flight WAPE={summary['rls_flight_energy_wh_wape_percent']:.4f}%、"
                f"完整窗口={summary['complete_window_count']}/{summary['window_count']}、"
                f"状态切换={summary['state_switch_count']}、耗时={elapsed:.2f}s"
            )
            log_result("RLS能量窗变量分析结果", {
                "stage": stage,
                "energy_window_seconds": window_seconds,
                "sample_power_wape_percent": summary["sample_power_w_wape_percent"],
                "rls_window_energy_wape_percent": summary["rls_window_energy_wh_wape_percent"],
                "rls_flight_energy_wape_percent": summary["rls_flight_energy_wh_wape_percent"],
                "complete_window_count": summary["complete_window_count"],
                "window_count": summary["window_count"],
                "state_switch_count": summary["state_switch_count"],
                "elapsed_seconds": round(elapsed, 3),
            })
    comparison = pd.DataFrame(rows)
    by_flight = pd.concat(flight_rows, ignore_index=True)
    comparison.to_csv(cfg.rls_energy_window_comparison_csv, index=False, encoding="utf-8")
    by_flight.to_csv(cfg.rls_energy_window_flight_csv, index=False, encoding="utf-8")
    payload = {
        "version": "3.2",
        "analysis_type": "rls_energy_window_variable_analysis",
        "energy_window_is_hyperparameter": False,
        "rls_tuning_candidate_count": int(len(rls_candidate_grid(cfg))),
        "fixed_rls_forgetting_factor": forgetting_factor,
        "fixed_rls_initial_covariance": initial_covariance,
        "fixed_rls_warmup_windows": 0,
        "comparison_windows_seconds": list(window_values),
        "comparison_file": str(cfg.rls_energy_window_comparison_csv),
        "by_flight_file": str(cfg.rls_energy_window_flight_csv),
        "rows": rows,
    }
    save_json(cfg.rls_energy_window_comparison_json, payload)
    return {
        "version": "3.2",
        "mode": "compare-rls-windows",
        "rls_tuning_candidate_count": int(len(rls_candidate_grid(cfg))),
        "fixed_rls_forgetting_factor": forgetting_factor,
        "fixed_rls_initial_covariance": initial_covariance,
        "comparison_windows_seconds": list(window_values),
...
def train_fixed_tcn(cfg: ExperimentConfig) -> dict:
    ...
    return {"version": "3.2", "mode": "train-fixed", "best_tcn_candidate": params["name"], "window_seconds": window_seconds, "window_steps": params["window_steps"], "epochs_run": len(logs), "best_epoch": checkpoint["best_epoch"], "best_val_loss": best_val, "model_file": str(cfg.best_model_file)}


def train_model(cfg: ExperimentConfig, stop_after_tcn: bool = False) -> dict:
    """功能: 按两阶段规则选择TCN窗口和秒级能量监督RLS参数并保存实验3.2产物。
    参数: cfg为实验配置对象。
    返回: 训练摘要字典。
    调用位置: main.py。
    """
    ...
    if stop_after_tcn:
        ...
        return {"version": "3.2", "mode": "tune-tcn", "best_tcn_candidate": best_params["name"], "best_tcn_window_seconds": best_params["window_seconds"], "best_tcn_window_steps": best_params["window_steps"], "best_tcn_selection_score": best_window_row["tcn_selection_score"], "tcn_candidate_count": len(window_rows), "model_file": str(cfg.best_model_file)}
    ...
    reference_window = float(cfg.rls_energy_window_seconds)
    print(f"阶段二：固定最优TCN窗口，参考能量窗固定为{reference_window:g}s（不参与搜索），搜索{rls_candidate_count}组RLS参数。")
    for rls_index, rls_params in enumerate(rls_candidates, start=1):
        candidate_started = time.perf_counter()
        runtime_params = {**rls_params, "energy_window_seconds": reference_window}
        corrected, _ = apply_rls_correction(best_window_val_base, val_df, scaler, cfg, rls_theta, update=True, rls_params=runtime_params)
        metrics = energy_window_selection_metrics(best_window_val_base, corrected, val_df, cfg, reference_window)
        row = {"phase": "stage2_rls", "tcn_window_seconds": best_params["window_seconds"], "tcn_window_steps": best_params["window_steps"], "reference_energy_window_seconds": reference_window, **rls_params, **metrics}
        rls_rows.append(row)
        if metrics["selection_score"] < best_rls_score:
            best_rls_score, best_rls = metrics["selection_score"], rls_params.copy()
        elapsed = time.perf_counter() - candidate_started
        print(
            f"RLS候选 {rls_index}/{rls_candidate_count} 测试结果："
            f"参考区间={reference_window:g}s，ff={rls_params['forgetting_factor']:g}，cov={rls_params['initial_covariance']:g}，"
            f"选择分数={metrics['selection_score']:.6f}，当前最优={best_rls_score:.6f}，耗时={elapsed:.2f}s"
        )
    if best_rls is None:
        raise RuntimeError("RLS参数搜索未得到可用候选。")
    print(f"RLS最优结果：候选={best_rls['candidate']}，遗忘因子={best_rls['forgetting_factor']:g}，初始协方差={best_rls['initial_covariance']:g}，参考能量窗={reference_window:g}s（仅用于调参，不是搜索变量），warmup固定为0个完整窗口，选择分数={best_rls_score:.6f}")
    ...
    best_runtime_params = {**best_rls, "energy_window_seconds": reference_window}
    val_online, _ = apply_rls_correction(val_base, val_df, scaler, cfg, None, update=True, rls_params=best_runtime_params, progress_label="在线RLS校准残差")
    val_static, _ = apply_rls_correction(val_base, val_df, scaler, cfg, rls_theta, update=False, rls_params=best_runtime_params, progress_label="固定RLS校准残差")
