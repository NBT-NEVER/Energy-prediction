...
# 开发时间: 2026-07-07
# 文件名: data_utils.py
# 功能说明: 下载无人机公开数据集并完成实验3.2的能耗特征工程、秒级能量聚合和数据切分
# 版本号：3.2
...
def prepare_dataset(cfg: ExperimentConfig, force: bool = False) -> dict:
    ...
    if not force and all(path.exists() for path in expected):
        ...
        if summary.get("version") == "3.2" and summary.get("second_energy_rows", 0) > 0:
            return summary
        print("检测到旧版或不完整处理数据，将重新生成3.2数据。")
    ...
    summary = {
        "version": "3.2",
        "_文件说明": (
            "本文件汇总实验3.2的数据来源、清洗结果、数据集划分规模、模型输入维度和预测目标。"
