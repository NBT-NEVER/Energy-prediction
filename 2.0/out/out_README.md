...
本实验采用基于留出验证集的超参数选择流程。数据按 `flight` 整组划分为训练集、验证集和测试集，同一个 flight 不会跨集合出现。三个集合不是按先后顺序简单地各运行一次，而是在不同阶段承担不同职责：训练集负责学习参数，验证集负责作出模型选择，测试集只在所有选择结束后报告最终性能。

| 数据集合 | 输入内容 | 在流程中的处理 | 产生的结果 | 是否更新 TCN 权重 |
| --- | --- | --- | --- | --- |
| 训练集 | 126 个 flight、158625 条记录及真实功率 | 计算标准化统计量；构造候选时间窗；反向传播训练 TCN；使用 TCN 训练集预测与真实功率拟合 RLS 全局初值 | 特征与目标标准化参数、各候选 TCN 权重、最终 TCN 权重、RLS 初始偏置和比例 | 是 |
| 验证集 | 28 个 flight、32820 条记录及真实功率 | 监控每轮验证损失；控制学习率和早停；比较五个时间窗的 TCN 与在线 RLS 输出；为最终模型生成区间校准残差 | 每个候选的 WAPE 和选择分数、最佳时间窗、最终最佳 epoch、在线/固定 RLS 校准残差 | 否 |
| 测试集 | 28 个 flight、36106 条记录及真实功率 | 加载已经确定的窗口、TCN 权重、标准化参数和 RLS 设置，只执行最终预测与评估 | 测试集逐点预测、功率指标、flight 能耗指标、分段统计和结果图 | 否 |
...
当前候选的训练集TCN预测 + 训练集真实功率
    -> 最小二乘拟合RLS初始偏置与比例

当前候选的验证集序列
    -> TCN输出验证集原始功率预测
    -> RLS按flight和时间顺序先校正、后读取真实功率更新
    -> 得到验证集在线RLS功率
    -> 分别计算TCN与RLS的样本功率WAPE和flight能耗WAPE
    -> 用RLS能耗WAPE + 0.2 x RLS功率WAPE得到候选选择分数

五个候选完成相同过程
    -> 选择分数最小的时间窗
```

验证集在这一阶段提供两类判据。候选内部以 `val_loss` 判断哪个 epoch 的 TCN 权重应当保留；候选之间以 RLS 校正后的 `selection_score` 判断哪个时间窗应当进入正式训练。这两个数值不能混用：`val_loss` 是标准化目标空间中的加权 Huber 损失，`selection_score` 是由验证集功率 WAPE 和 flight 能耗 WAPE 组成的系统级评价量。

时间窗口确定后，候选阶段的临时 TCN 权重不直接作为最终模型。程序按最佳窗口重新初始化 TCN，再次使用训练集执行正式训练。验证集仍在正式训练中工作，但不进入梯度计算：每轮训练结束后计算验证损失，`ReduceLROnPlateau` 根据该损失决定是否降低学习率，连续 `patience` 轮未产生更低验证损失时触发早停。程序最终恢复验证损失最低轮次的权重，而不是直接保存停止时最后一轮的权重。

正式训练结束后，训练集还有一次用途。最终 TCN 对训练集做前向预测，程序把训练集 TCN 功率与训练集真实功率组成回归样本，拟合 RLS 的全局偏置和比例初值。这里得到的是 RLS 开始一段新 flight 时的基础状态，不是验证集或测试集上的最终校正结果。

验证集随后用于置信区间校准。最终 TCN 先产生验证集原始预测，再分别形成两条残差链：在线 RLS 按“先输出当前校正值、再用当前真实功率更新参数”运行；固定 RLS 使用训练集拟合的初值但不读取验证集真实功率更新。两条预测分别与验证集真实功率相减并取绝对值，保存到 `uncertainty_calibration_2.0.npz`。用户修改置信度时，程序从对应残差序列读取保序分位数，不需要重新训练或再次执行 TCN。

测试集只在窗口、最终 epoch、TCN 权重、RLS 设置和区间校准全部确定后进入流程。测试集真实功率用于计算误差和模拟有反馈条件下的在线 RLS 更新，不参与窗口选择、学习率调整、早停或模型参数回改。由测试结果生成的 `test_predictions_2.0.csv`、`evaluation_2.0.json`、flight 能耗汇总和功率分段表，属于最终独立评估产物。

完整的信息流可压缩为：

```text
训练集 -> 学习TCN权重、计算标准化参数、拟合RLS初值
验证集 -> 选候选窗口、选最终epoch、控制早停与学习率、标定预测区间
测试集 -> 固定方案后的最终预测、误差统计与图表输出
```

两张损失图使用同一组最终训练记录，但生成时机不同。`loss_curve_2.0.png` 由 `train.py` 在训练结束时直接读取内存中的 `logs` 绘制；`training_loss_history.png` 由 `visualize.py` 读取已经保存的 `training_log_2.0.csv` 再次绘制。两图的横轴都是 epoch，纵轴都是加权 Huber 损失，曲线分别为 `train_loss` 和 `val_loss`。它们不包含五个候选窗口的调参训练过程，只反映最佳窗口确定后的正式 TCN 训练。前者用于训练结束后立即查看，后者属于统一可视化输出，数据含义相同。
