...
本次 72 组搜索选择：遗忘因子 `0.97`、初始协方差 `10`、预热 `0.1 s`。汇总价值函数为 `6.68360866`，标准差和 28 个验证 flight 的逐组结果记录位于 `out/model/rls_tuning_results_3.0.csv`。旧网格的最优点曾撞在遗忘因子 0.95、协方差 100、预热 0.5 s 的边界，因此新网格向更低遗忘因子、更低协方差和更短预热扩展；扩展是为了消除边界截断，不预设新范围必然更优。
