...
# 开发时间: 2026-07-08
# 文件名: visualize.py
# 功能说明: 生成实验3.0的TCN、RLS评估和自定义工况可视化图表
# 版本号：3.0
...
def plot_prediction_outputs(cfg: ExperimentConfig, max_flights: int = 3) -> list[Path]:
    ...
    if {"second_window", "actual_second_energy_wh", "predicted_second_energy_wh"}.issubset(predictions.columns):
        selected = predictions["flight"].drop_duplicates().sort_values().head(max_flights)
...
def normalize_custom_input(frame: pd.DataFrame, cfg: ExperimentConfig) -> pd.DataFrame:
    ...
    if invalid_routes:
        raise ValueError(f"实验3.0自定义工况仅支持{cfg.training_route}航线，输入包含: {invalid_routes}")
    frame["route"] = cfg.training_route
    for column in feature_columns:
        if column.startswith("route_"):
