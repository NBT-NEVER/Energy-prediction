...
# 开发时间: 2026-07-07
# 文件名: data_utils.py
# 功能说明: 下载无人机公开数据集并完成实验3.0的能耗特征工程、秒级能量聚合和数据切分
# 版本号：3.0
...
def aggregate_second_energy(frame: pd.DataFrame) -> pd.DataFrame:
    """功能: 按每个flight的相对时间和不规则采样间隔聚合真实秒级能量。
    参数: frame为已计算power_w、dt_seconds且按flight和time排序的采样表。
    返回: 每个flight每个时间窗口一行的秒级能量表。
    调用位置: prepare_dataset、训练和评估流程。
    """

    work = frame[["flight", "time", "dt_seconds", "power_w"]].copy()
    work["second_window"] = np.floor(work["time"].to_numpy(dtype=float)).astype(int)
    work["sample_energy_wh"] = work["power_w"] * work["dt_seconds"] / 3600.0
    grouped = work.groupby(["flight", "second_window"], sort=True)
    energy = grouped.agg(
        window_start_s=("time", "min"),
        sample_count=("power_w", "size"),
        window_duration_s=("dt_seconds", "sum"),
        true_energy_wh=("sample_energy_wh", "sum"),
    ).reset_index()
    energy["window_end_s"] = energy["window_start_s"] + energy["window_duration_s"]
    energy["cumulative_true_energy_wh"] = energy.groupby("flight", sort=False)["true_energy_wh"].cumsum()
    return energy
...
def prepare_dataset(cfg: ExperimentConfig, force: bool = False) -> dict:
    ...
    if not force and all(path.exists() for path in expected):
        ...
        if summary.get("version") == "3.0" and summary.get("second_energy_rows", 0) > 0:
            return summary
        print("检测到旧版或不完整处理数据，将重新生成3.0数据。")
    ...
    second_energy.to_csv(cfg.second_energy_csv, index=False, encoding="utf-8")
    ...
    meta = {
        ...
        "field_descriptions": {
            ...
            "second_window": "按flight相对时间floor(time)划分的秒级窗口编号。",
            "true_energy_wh": "该flight该秒窗口内按不规则采样间隔积分得到的真实能量，单位为瓦时。",
            "cumulative_true_energy_wh": "该flight从起始窗口累计到当前窗口的真实能量，单位为瓦时。",
    ...
    summary = {
        "version": "3.0",
        "_文件说明": (
            "本文件汇总实验3.0的数据来源、清洗结果、数据集划分规模、模型输入维度和预测目标。"
        ...
        "second_energy_rows": int(len(second_energy)),
...
def prepare_prediction_frame(input_csv: Path, cfg: ExperimentConfig) -> tuple[pd.DataFrame, list[str]]:
    ...
    if "route" in frame.columns:
        ...
        if invalid_routes:
            raise ValueError(f"实验3.0仅支持{cfg.training_route}航线，输入文件包含: {invalid_routes}")
    else:
        ...
        if active_dummies:
            raise ValueError(f"实验3.0仅支持{cfg.training_route}航线，输入特征包含其他航线独热编码: {active_dummies}")
