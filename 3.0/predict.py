...
# 开发时间: 2026-08-31
# 文件名: predict.py
# 功能说明: 执行实验3.0的TCN前向推理和秒级能量监督RLS实时功率校正
# 版本号：3.0
...
def predict_from_csv(
    ...
    if "dt_seconds" in output.columns:
        ...
        output["second_window"] = np.floor(output["time"].to_numpy(dtype=float)).astype(int)
        grouped = output.groupby(["flight", "second_window"], sort=False)
        output["tcn_second_energy_wh"] = grouped["tcn_predicted_energy_wh"].transform("sum")
